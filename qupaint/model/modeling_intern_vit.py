...
try:  # only needed by the FlashAttention path
...
except ImportError:  # stochastic depth is training-only; released configs use 0.0
    class DropPath(nn.Identity):
        def __init__(self, drop_prob: float = 0.0):
            super().__init__()
            if drop_prob > 0.0:
                raise ImportError(
                    "drop_path_rate > 0 requires `timm`; install it or set drop_path_rate=0."
                )
