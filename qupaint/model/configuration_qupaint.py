...
class QuPAINTConfig(PretrainedConfig):
    model_type = "cviu_chat"
    is_composition = True
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(vision_config, dict):
            if vision_config["architectures"][0] == "InternVisionModel":
                self.vision_config = InternVisionConfig(**vision_config)
            elif vision_config["architectures"][0] == "CLIPVisionModel":
                self.vision_config = CLIPVisionConfig(**vision_config)
            else:
                raise ValueError(
                    "Unsupported architecture: {}".format(
                        vision_config["architectures"][0]
                    )
                )
        else:
...
CVIUVLChatConfig = QuPAINTConfig
